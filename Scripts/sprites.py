...
class Player(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if not self.dead:
            ...
            if not self.jumping and (keys[pg.K_s] or keys[pg.K_DOWN]):
                self.crouching = True
            else:
                self.crouching = False

            if keys[pg.K_a] or keys[pg.K_LEFT] and not self.crouching:
                if not self.crouching:
                    self.acc.x = -playerAcc
                    self.facing = "Left"
                    if keys[pg.K_LSHIFT] or keys[pg.K_RSHIFT]:
                        self.acc.x = playerAcc * -2
            if keys[pg.K_d] or keys[pg.K_RIGHT] and not self.crouching:
                if not self.crouching:
                    self.acc.x = playerAcc
                    self.facing = "Right"
                    if keys[pg.K_LSHIFT] or keys[pg.K_RSHIFT]:
